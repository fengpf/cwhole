...
uint32_t* node_parent(void* node) {
    return (uint32_t*) ((uint32_t*)(node) + PARENT_POINTER_OFFSET);
}

uint32_t* internal_node_num_keys(void* node) {
    return (uint32_t*) ((uint32_t*)(node) + INTERNAL_NODE_NUM_KEYS_OFFSET);
}

uint32_t* internal_node_right_child(void* node) {
    return (uint32_t*) ((uint32_t*)(node) +INTERNAL_NODE_RIGHT_CHILD_OFFSET);
}

uint32_t* internal_node_cell(void* node, uint32_t cell_num) {
    return (uint32_t*) ((uint32_t*)(node) + INTERNAL_NODE_HEADER_SIZE + cell_num * INTERNAL_NODE_CELL_SIZE);
}
...
uint32_t* leaf_node_num_cells(void* node) {
    return (uint32_t*) ((uint32_t*)(node)  + LEAF_NODE_NUM_CELLS_OFFSET);
}

uint32_t* leaf_node_next_leaf(void* node) {
    return (uint32_t*) ((uint32_t*)(node)  + LEAF_NODE_NEXT_LEAF_OFFSET);
}
